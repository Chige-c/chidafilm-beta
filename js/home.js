...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!history.state) {
        ...
        const currentURL = window.location.search ? window.location.href : window.location.pathname + '?view=home';
...
function switchTab(type, clickedBtn, tabName) {
    ...
    if (type === 'MyList') {
        if (homeView) homeView.style.display = 'none';
        if (heroSection) heroSection.style.display = 'none';
        if (mobileDots) mobileDots.style.display = 'none';
        if (mylistView) mylistView.style.display = ''; 
    } else {
        if (mylistView) mylistView.style.display = 'none';
        if (homeView) homeView.style.display = ''; 
        if (heroSection) heroSection.style.display = ''; 
        if (mobileDots) mobileDots.style.display = '';
    }
    ...
    const newUrl = (tabName === 'home' && type === '') ? window.location.pathname + '?view=home' : window.location.pathname + `?tab=${tabName}`;

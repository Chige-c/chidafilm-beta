...
async function loadResumeItems() {
    ...
    card.dataset.type = item.Type || 'Movie';
                ...
                const thumbUrl = `${SERVER_URL}/Items/${thumbId}/Images/Thumb?fillWidth=700&quality=100`;
                const backdropUrl = `${SERVER_URL}/Items/${thumbId}/Images/Backdrop/0?fillWidth=700&quality=100`;
    ...
    try {
        ...
        if (data && (data.Items || Array.isArray(data))) {
            ...
            items.forEach(item => {
                ...
                const thumbUrl = `${SERVER_URL}/Items/${targetId}/Images/Thumb?fillWidth=700&quality=100`;
                const backdropUrl = `${SERVER_URL}/Items/${targetId}/Images/Backdrop/0?fillWidth=700&quality=100`;
                const primaryUrl = `${SERVER_URL}/Items/${targetId}/Images/Primary?fillWidth=700&quality=100`;
...
async function loadTrendingFromTMDB(gridId, fetchUrl, showRank = false) {
    try {
        ...
        matchedItems.slice(0, 15).forEach((item, index) => {
            ...
            const thumbUrl = `${SERVER_URL}/Items/${item.Id}/Images/Thumb?fillWidth=700&quality=100`;
            const backdropUrl = `${SERVER_URL}/Items/${item.Id}/Images/Backdrop/0?fillWidth=700&quality=100`;

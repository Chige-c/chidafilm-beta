...
async function loadAndPlayHls(startTime, fetchNewInfo = false) {
    ...
    `AudioBitrate=320000&TranscodingMaxAudioChannels=2&SegmentContainer=fmp4&_t=${Date.now()}`;
    ...
    if (Hls.isSupported()) {
        ...
        // 🌟 追加：エラー発生時の自動リカバリ処理（音ズレやループ対策）
        hlsInstance.on(Hls.Events.ERROR, function (event, data) {
            if (data.fatal) {
                switch (data.type) {
                    case Hls.ErrorTypes.NETWORK_ERROR:
                        console.warn("🌐 [HLS] ネットワークエラー発生。ストリームを再読込します...", data.details);
                        hlsInstance.startLoad();
                        break;
                    case Hls.ErrorTypes.MEDIA_ERROR:
                        if (data.details === "manifestIncompatibleCodecsError") {
                             // 再生互換性エラーの場合は無理に修復しない
                             console.error("❌ [HLS] 再生互換性エラーです。形式を確認してください。");
                             hlsInstance.destroy();
                             return;
                        }
                        console.warn("🎬 [HLS] メディアエラー（音ズレ・ループ原因）。自動リカバリを実行します...", data.details);
                        hlsInstance.recoverMediaError();
                        break;
                    default:
                        console.error("❌ [HLS] 致命的なエラーのため、ストリームを停止しました。", data.details);
                        hlsInstance.destroy();
                        break;
                }
            }
